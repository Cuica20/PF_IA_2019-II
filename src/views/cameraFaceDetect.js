...
class CameraFaceDetect extends Component {
  ...
  // AMPLIO EL INTERVALO DE ESPERA A 2 SEG
  startCapture = () => {
  ...
  capture = async () => {
    if (!!this.webcam.current) {
      await getFullFaceDescription(
        this.webcam.current.getScreenshot(), WIDTH, HEIGHT,
        inputSize
  ...
  render() {
    ...
    if (!!facingMode) {
      ...
      if (facingMode === 'user') {camera = 'Frontal';
      } else {camera = 'Trasera';}
    ...
    return (
      <div
        ...
        <p>Cámara: {camera}, rostros detectados: {this.state.fullDesc.length}</p>
        <div
          style={{
            width: WIDTH,
            height: HEIGHT
